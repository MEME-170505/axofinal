...
// Watch para cargar datos del plan cuando se abre el modal
watch(() => props.plan, (newPlan) => {
  if (newPlan) {
    formData.value = {
      id: newPlan.id,
      nombre: newPlan.nombre || newPlan.name,
      servicios: {
        diseno: newPlan.detalles?.includes('DISEÑO UI/UX') || false,
        web: newPlan.detalles?.includes('PÁGINA WEB') || false,
        correo: newPlan.detalles?.includes('CORREO EMPRESARIAL') || false,
        marketing: newPlan.detalles?.includes('MARKETING DIGITAL') || false,
        automatizaciones: newPlan.detalles?.includes('AUTOMATIZACIONES') || false
      },
      precio: parseFloat(newPlan.precio?.replace(/[^0-9]/g, '') || 0)
    }
  }
}, { immediate: true })
...
const handleSubmit = () => {
  ...
  const planData = {
    ...
    precio: `$${formData.value.precio} MXN`,
